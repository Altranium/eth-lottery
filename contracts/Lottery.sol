...
contract Lottery {
    ...
    function random() private view returns(uint) {
        return uint(keccak256(block.difficulty, block.timestamp, players));    // Uses block & now global variable
    }

    function pickWinner() public {
        uint index = random() % players.length;
        players[index].transfer(this.balance);      // Transfer the contract's balance to the winner
    }
}